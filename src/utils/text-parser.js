      ...
      if (triggers.includes(letter)) {
        ...
        if (startTag && !shouldNotBeNestedAndAlreadyIsStartTag) {
          if (history[currentKey].type !== 'normal') {
            history.nodes = [...history.nodes, tags[history[currentKey].type]
              .render(history[currentKey].text,
                history[currentKey].tag)]
          ...
          shouldAdd = false
          skipTillIndex = startTag.skipLength ?
            i + startTag.skipLength : i + startTag.tag.length - 1
        } else if (endTag) {
          if (history[currentKey].type !== 'normal') {
            if (currentKey === 0) {
              if (tags[Object.keys(tags)[endTag.i]].disableNesting) {
                nodes.push(tags[history[currentKey].type].render(history[currentKey].text,
                  history[currentKey].tag))
              } else {
                history.nodes = [...history.nodes,
                  tags[history[currentKey].type].render(history[currentKey].text,
                    history[currentKey].tag)]
                nodes.push(tags[history[currentKey].type]
                  .render(history.nodes, history[currentKey].tag))
              }
              history[currentKey] = {
                text: '',
                type: 'normal'
