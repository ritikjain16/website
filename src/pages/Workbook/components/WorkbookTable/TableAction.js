...
import { deleteWorkbook } from '../../../../actions/workbook'
...
const TableAction = (props) => {
  ...
  const deleteAction = async () => {
    setSubmitting(true)
    setShowPopup(true)
    const { deleteWorkbook: data } = await deleteWorkbook(id)
    if (data && data.id) {
      setSubmitting(false)
      setShowPopup(false)
    } else {
  ...
  return (
    ...
    >
      ...
      <WorkbookStyle.StyledDivider type='vertical' />
      <MainTable.ActionItem.IconWrapper>
        <Popconfirm
          title='Do you want to delete this workbook ?'
          placement='topRight'
          visible={showPopup}
