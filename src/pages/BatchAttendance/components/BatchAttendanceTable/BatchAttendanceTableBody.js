...
class BatchAttendanceTableBody extends React.Component {
  ...
  startSession = ({ batchSessionId, input }) => {
    updateBatchSession({ batchSessionId, input })
  }
            ...
            >
              ...
              >
                <MainTable.Item>
                  ...
                  >
                    <span style={{ padding: '5px', flex: 0.6 }} >{!item.batch ? '-' : item.batch.code}{`(${!item.batch || !item.batch.studentsMeta ? '' : item.batch.studentsMeta.count})`}</span>
                    <Link
                      rel='noopener noreferrer'
                      to={`/ums/assignTimetable/${!item.batch ? '' : item.batch.code}`}
                    >
                      <Tooltip
                        placement='top'
                    ...
                    <Link
                      rel='noopener noreferrer'
                      to={`/ums/batchMapping/${!item.batch ? '' : item.batch.code}`}
                    >
                      <Tooltip
                        placement='top'
