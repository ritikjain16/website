...
export default class CompletedSessionsTags extends Component {
  ...
  renderTags = () => {
    ...
    return tags.map((item, key) =>
      ...
      >
        <CompletedSessionStyle.TagsIcon style={{ marginRight: '5px', backgroundColor: `${this.renderZoneColor(item.zone)}` }} >{item.icon}</CompletedSessionStyle.TagsIcon>
        <span>{item.displayTitle ? item.displayTitle : item.tag}</span>
        {
          count.data[item.tag] && count.data[item.tag].count !== 0 ?
            <CompletedSessionStyle.TagsIcon style={{ marginLeft: '5px', backgroundColor: '#8359f7' }} >{count.data[item.tag].count}</CompletedSessionStyle.TagsIcon>
            : null
        }
      </CompletedSessionStyle.StyledTagButton>)
  }
