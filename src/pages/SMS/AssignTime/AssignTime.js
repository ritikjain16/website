...
class AssignTimeTable extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedCourse: '',
      orderToFetch: 1,
      selectedTopicOrder: 1,
      batchType: '',
      selectedMentorId: ''
    }
  }
  componentDidMount() {
    // batchCode from params if reached page by link
    ...
    if (getDataFromLocalStorage('login.role') === MENTOR) {
      ...
      this.setState({
        mentorUserId: id,
        currentMentorId: id
      }, () => fetchBatchCodes(id, '{type_not : normal}'))
    } else {
      fetchBatchMentor(this.state.currentBatchCode)
      fetchMentorsList()
    }
  ...
  componentDidUpdate(prevProps) {
    ...
    // if mentor of batch fetched, get batch details from batchMentor
    if (this.state.paramBatchCode && prevProps.isFetchingMentorOfBatch && this.props.hasFetchedMentorOfBatch) {
      const mentorId = !mentorOfBatch ? null : get(mentorOfBatch.toJS(), '[0].allottedMentor.id', '')
      ...
      const batchType = !mentorOfBatch ? null : get(mentorOfBatch.toJS(), '[0].type', '')
      this.setState({
    ...
    if (prevProps.isFetchingBatchSessions && this.props.hasFetchedBatchSessions) {
      const batchesFiltered = !batchSessionsData ? [] : batchSessionsData.toJS()
      this.setState({
        batchSessionsData: batchesFiltered,
        topic: topic ? topic.toJS() : [],
      })
      if (batchesFiltered.length === 0) {
        this.setState({
          topicOrder: null
        })
      }
    ...
    if (prevProps.isFetchingLastSessionOfBatch && this.props.hasFetchedLastSessionOfBatch) {
    ...
    if (prevProps.isFetchingMentors && this.props.hasFetchedMentors) {
      ...
      mentorsFiltered = mentorsFiltered.filter(item => item.mentorProfile !== null)
      this.setState({
    ...
    if (!isFetchingTopicId && prevProps.isFetchingTopicId) {
      ...
      if (this.state.actiontype === 'edit' && selectedCourse === get(sessionToEdit, 'course.id')) {
        ...
        selectedTopicOrder = get(sessionToEdit, 'topic.order')
      } else {
        topicSelected = this.props.topicId ? this.props.topicId.toJS()[0].id : null
        selectedTopicOrder = this.props.topicId ? this.props.topicId.toJS()[0].order : 0
      }
      this.setState({
    ...
    if (!isFetchingCheckMentorSession && prevProps.isFetchingCheckMentorSession) {
      this.setState({
    ...
    // after adding a mentor session, have to either add or update batch session with new mentor session deets
    if (prevProps.isAddingMentorSession && this.props.hasAddedMentorSession) {
      const { editingSession, slotsSelected, dateSelected, sessionToEdit } = this.state
      const slotStatus = []
      ...
      if (this.state.actiontype === 'add') {
        ...
        // update batch session with mentor session connect id, when we change the mentor
        updateBatchSession(
          sessionToEdit.id,
  ...
  // handle batch change from dropdown, fetch batch sessions and last session of new batch
  handleBatchChange = (value) => {
    const batch = this.props.batches.toJS().filter(item => item.code === value)
    this.setState({
      currentBatchCode: value,
      currentBatchId: batch[0].id,
      batchType: batch[0].type
    }, () => {
      fetchBatchSessions(this.state.currentBatchCode, {
        page: this.state.currentPage,
        perPage: this.state.perPageQueries
      ...
      fetchLastSessionOfBatch(this.state.currentBatchId)
      window.history.pushState({}, '', `/sms/assignTimetable/${value}`)
    })
  }
  // handle topic change from dropdown in modal, set order
  handleTopicChange = (value) => {
    const { topicId } = this.state
    const selectedTopicOrder = topicId.find(topic => get(topic, 'id') === value)
    this.setState({
      topicSelected: value,
      selectedTopicOrder: get(selectedTopicOrder, 'order')
    })
  }
  // handle mentor change in dropdown
  handleMentorChange = (value) => {
    this.setState({
      currentMentorId: value,
      batchName: [],
      currentBatchCode: '',
    }, () => fetchBatchCodes(value, '{type_not : normal}'))
  }
  // clear selections when modal cancel
  handleModalCancel = () => {
    this.setState({
      modalVisible: false,
      completedsessionerror: false,
  ...
  showModal = (actiontype, data) => {
    const { batchSessionsData, coursesList, batchType } = this.state
    let orderToFetch = 1
    // if editing modal && saved topic order in state, pick this order directly for next topics fetch
    if (actiontype === 'edit' && this.state.topicOrder && this.state.topicOrder[0]) {
      orderToFetch = this.state.topicOrder[0].order
    } else if (actiontype === 'add' && this.state.topicOrder && this.state.topicOrder[0]) {
    ...
    this.setState({
      actiontype,
      modalVisible: true,
      mentorUserId: this.state.batchSessionsData && this.state.batchSessionsData[0]
        ? get(this.state.batchSessionsData, '[0].batch.allottedMentor.id')
      ...
      if (batchType === 'b2b2c' && orderToFetch === 1) sessionType = 'trial'
      else sessionType = 'batch'
      checkMentorSession(this.state.mentorUserId, new Date(date).toISOString(), sessionType)
    })
    if (actiontype === 'edit') {
      this.setState({
        sessionToEdit: data,
  ...
  renderDateTablet = () => {
    ...
    let sessionType = ''
    const { selectedTopicOrder, batchType } = this.state
    if (batchType === 'b2b2c' && selectedTopicOrder === 1) sessionType = 'trial'
    else sessionType = 'batch'
    if (this.state.actiontype === 'add') {
      return (
        <div>
          <h3 style={{ textAlign: 'center' }}>TimeTable</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 1fr 1fr', gridGap: '15px' }}>
            {
              days.map(day => (
                <SessionTimeModalStyle.DateTablet
                  onClick={() => this.setState({
                    ...
                    () => {
                      checkMentorSession(this.state.mentorUserId, new Date(this.state.dateSelected).toISOString(), sessionType)
                    })}
                  selected={
      ...
      return <h1 style={{ textAlign: 'center' }}>Are sure you want delete it?</h1>
  ...
  render() {
    const { Option } = Select
    const { isFetchingBatchSessions, isAddingBatchSession,
      isDeletingBatchSession, isUpdatingBatchSession,
      hasAddedBatchSession, hasDeletedBatchSession,
      isFetchingTopicId, hasFetchedBatchSessions,
      hasUpdatedBatchSession, batchSessionsCount } = this.props
    const date = changedatetoisostring(offsetDate(this.state.dateSelected, 1, 'ADD'))
    const { currentPage, perPageQueries,
      selectedCourse, coursesList, orderToFetch, selectedMentorId } = this.state
    const savedRole = getDataFromLocalStorage('login.role')
    return (
      <div>
        <Modal
          title={this.state.actionType === 'delete' ? 'Delete Session' : 'Assign TimeTable'}
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            {/*
              show the mentor select option in modal, to change mentor for particular batch session
            */}
            ...
            <div>
              <h2>
                {
                  savedRole === MENTOR ? getDataFromLocalStorage('login.name') : <div />
                }
              </h2>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', paddingBottom: '25px' }}>
          <div style={{ paddingLeft: '5px' }}>
            {
              savedRole !== MENTOR ? <h4>Select Mentor</h4> : <div />
            }
            {
              savedRole !== MENTOR ?
                <Select
