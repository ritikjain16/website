...
class Payment extends Component {
  ...
  disabledDate = (current) => {
    const { currentDueDate } = this.state
    return current && current < moment(currentDueDate).startOf('day');
  }
  ...
  renderPaidInstallmentDetails = () => {
    ...
    return installmentsDone.map((installments, index) =>
        ...
        >
          ...
          >
            {index + 1}{this.getSuffix(index + 1)} installment of Rs. {installments.amount} paid on {moment(get(installments, 'paidDate')).format('Do [of] MMMM [,] YYYY')}
          </SubTitle>
        </div>
      )
  }
    ...
    return (
      <Fragment>
        <SubTitle style={{ color: '#73a5aa' }}>
          <span>{currentInstallment + 1}{this.getSuffix(currentInstallment + 1)} installment</span>
          {
          ...
          >
            ...
            {/* <Col span={8} offset={2} >
            <CustomSelect defaultValue='Select Amount' suffixIcon={<CopyOutlined />}>
                {
                  paymentLinks.map((link) =>
                    <Option value={link.id}>
                      <SubTitle>{link.amount ? link.amount : this.state.amountPerInstallment}</SubTitle>
                    </Option>
                  )
                }
              </CustomSelect>
  ...
  renderBalance() {
    ...
    const { installmentsDueDate } = this.state
    ...
    // const installmentNumber = get(paymentPlan[0], 'installmentNumber')
    // if (balance === installmentNumber) {
    //   return null
    // }
    return balance
      .map((details, index) => (
  ...
  render() {
    const { paymentPlan } = this.props.data
    const { amountPerInstallment, totalAmountCollected } = this.state
    if (!paymentPlan || paymentPlan.length === 0) {
      return (
        <p
          style={{
    ...
    // console.log(paymentPlan[0])
    const { Hl, Title, NoticeBox, SubTitle, PaymentSection, Banner } = MentorConversionStyle
    // this.renderTotalAmount()
    return (
      <PaymentSection>
        <Banner>
          Total Amount Collected: {commaNumber(totalAmountCollected)}
        </Banner>
        <Row style={{ textAlign: 'center' }}>
          <Col>
            <Title>
              Total amount payable: {commaNumber(get(paymentPlan[0], 'finalSellingPrice'))}
            </Title>
          </Col>
        </Row>
        <Row>
          <Col>
            <SubTitle style={{ textAlign: 'center', justifyContent: 'center' }}>
              {commaNumber(amountPerInstallment)} x {get(paymentPlan[0], 'installmentNumber')}{' '}
              installments
            </SubTitle>
          </Col>
        </Row>
