...
import { ADMIN, MENTOR, SALES_EXECUTIVE, SMS, UMS_ADMIN, UMS_VIEWER } from '../../../constants/roles'
import getDataFromLocalStorage from '../../../utils/extract-from-localStorage'
import { Table, Select, Input, Button, DatePicker, Pagination, Switch } from 'antd'
...
import fetchMentorConversionOfSchool from './../../../actions/mentorConversionOfSchool/fetchMentorConversionOfSchool'
import fetchTotalAmount from './../../../actions/mentorConversion/fetchTotalAmount'
...
class MentorConversionOfSchool extends Component {
  state = {
    tableData: [],
    studentData: null,
    searchKey: 'all',
  ...
  componentDidMount() {
    ...
    if (savedRole === SALES_EXECUTIVE) {
      ...
      fetchMentorForSalesExecutive(savedId).then(res => {
  ...
  componentDidUpdate(prevProps) {
    ...
    const prevLinks = prevProps.userPaymentLinks
    const newLinks = this.props.userPaymentLinks
    if (
      dataFetchStatus &&
      dataFetchStatusPrev &&
    ...
    } else if (prevLinks != newLinks) {
      this.convertDataForStudentColumn()
    } else {
      const dataFetchStatus = this.props.userPaymentPlans
      const dataFetchStatusPrev = prevProps.userPaymentPlans
      // console.log(dataFetchStatus && dataFetchStatusPrev && dataFetchStatusPrev !== dataFetchStatus)
      if (dataFetchStatus && dataFetchStatusPrev && dataFetchStatusPrev !== dataFetchStatus) {
        this.convertDataForStudentColumn()
        fetchTotalAmount(this.state.totalAmountInput)
      }
    }
    if (prevProps && prevProps.userPaymentPlans !== this.props.userPaymentPlans) {
      fetchTotalAmount(this.state.totalAmountInput)
    }
    ...
    filter(salesOperationInJS, item => get(item, 'leadStatus') === 'won').forEach(session => {
      ...
      tableData.push({
        ...
        payment: {
          salesId: session.id,
          paymentPlan: filter(userPaymentPlans, item => get(item, 'salesOperation.id') === get(session, 'id')),
          paymentLinks: get(this.props, 'userPaymentLinks').toJS(),
          menteeId: get(session, 'client.id')
        }
      })
    })
  ...
  callFetchQueryForFiltering = () => {
    const { searchKey, filterQuery, searchValue, perPageQueries, currentPage, mentorsId, totalAmountInput } = this.state
    const savedRole = getDataFromLocalStorage('login.role')
    const savedId = getDataFromLocalStorage('login.id')
    if (searchKey === 'clearSrch') {
      if (savedRole === MENTOR) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, [savedId], {
          dateFilterQuery: filterQuery.dateFilterQuery
        }, this.state.bringOverDueSessions)
       } else if (savedRole === SALES_EXECUTIVE) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, mentorsId, {
          dateFilterQuery: filterQuery.dateFilterQuery
        }, this.state.bringOverDueSessions)
       } else {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, null, {
          dateFilterQuery: filterQuery.dateFilterQuery
        }, this.state.bringOverDueSessions)
      }
      ...
      if (savedRole === MENTOR) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, [savedId],
          filterQuery, this.state.bringOverDueSessions)
      } else if (savedRole === SALES_EXECUTIVE) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, mentorsId,
          filterQuery, this.state.bringOverDueSessions)
      } else {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, null, filterQuery, this.state.bringOverDueSessions)
      }
      ...
      if (savedRole === MENTOR) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, [savedId], filterQuery, this.state.bringOverDueSessions)
      } else if (savedRole === SALES_EXECUTIVE) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage: 1 }, mentorsId,
          filterQuery, this.state.bringOverDueSessions)
      } else {
      ...
      if (savedRole === MENTOR) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage }, [savedId], filterQuery, this.state.bringOverDueSessions)
      } else if (savedRole === SALES_EXECUTIVE) {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage }, mentorsId,
          filterQuery, this.state.bringOverDueSessions)
      } else {
        fetchMentorConversionOfSchool({ perPageQueries, currentPage }, null, filterQuery, this.state.bringOverDueSessions)
      }
    }
    fetchTotalAmount(this.state.totalAmountInput)
  }
  ...
  onPageChange = page => {
    this.setState(
      ...
      () => {
        const { filterQuery, perPageQueries } = this.state
        var { currentPage, mentorsId } = this.state
        const savedRole = getDataFromLocalStorage('login.role')
        const savedId = getDataFromLocalStorage('login.id')
        if (savedRole === MENTOR) {
          fetchMentorConversionOfSchool({ perPageQueries, currentPage }, [savedId], filterQuery, this.state.bringOverDueSessions)
          ...
          fetchMentorConversionOfSchool({ perPageQueries, currentPage }, null, filterQuery,
            this.state.bringOverDueSessions)
        }
      }
    )
  ...
  render() {
    ...
    const { Column } = Table
    const { Option } = Select
    const { StyledTable, SearchBox } = MentorConversionStyle
    const { openFinancialModal } = this.state
    return (
      <Fragment>
        <MentorMenteeManagement current={2} totalCount={totalConverted} />
        <div
          style={{
            minWidth: 1345
          }}
          ...
          <SearchBox>
            <div>
              ...
              >
                ...
                {admin ? <Option value="Mentor Name">Mentor Name</Option> : null}
        ...
        >
          ...
          />
            <Column
              ...
              render={(text, record, index) => {
                // console.log(text)
                return <Payment data={text} plans={this.props.userPaymentPlans.toJS()} />
              }}
            />
          </StyledTable>
          <div
...
export default MentorConversionOfSchool
