...
import { Row, Col, Select, Input, DatePicker, Switch } from 'antd'
...
class Plan extends Component {
  ...
  setDefaultPlan = () => {
    if (!get(this.props, 'data.paymentPlan[0]')) {
      let currency = '₹'
      if (get(this.props, 'products', []).length > 0) {
        currency = get(this.props, 'products[0].price.currency') === 'RS' ? '₹' : '$'
      }
      this.setState({
        addUserPaymentPlan: {
          sessionsPerMonth: 6,
      ...
      let currency = get(this.props, 'data.paymentPlan[0].product.price.currency') === 'RS' ? '₹' : '$'
      ...
      `)
          .then(res => {
            if (res.data.discounts && res.data.discounts.length) {
              const { percentage, expiryDate } = res.data.discounts[0]
              // if expiry date is before current Date the show coupon code expired
  ...
  getModalTypeOptions = () => {
    ...
    if (products && products.length > 0) {
      MODAL_TYPE.forEach(type => {
        const typesArray = products.map(product => get(product, 'type'))
        if (typesArray.includes(type)) {
          newModalTypes.push(type)
        }
      })
    }
  ...
  render() {
    const { addUserPaymentPlan, currency } = this.state
      ...
      <Fragment>
        ...
        <Row>
          ...
          >
            ...
            >
              {
                this.getModalTypeOptions().map(modal => (
        ...
        {this.state.productConnectId ? (
          <Fragment>
            <Row>
              ...
              >
                {currency}{' '}{get(addUserPaymentPlan, 'productPrice')}
              </Col>
            </Row>
            <Row>
              <Col
              ...
              >
                <SubTitle>
                  {currency}{' '}{get(this.state.product, 'discounts[0].percentage') ? Math.ceil(
                    -(get(addUserPaymentPlan, 'productPrice') / 100) *
            ...
            {addUserPaymentPlan.installmentType === 'auto' ? (
              ...
              <Row>
                ...
                >
                  {currency} {this.getManualSellingPrice()}
                </Col>
              </Row>
            )}
            <Hl />
            <Row>
              ...
              >
                {addUserPaymentPlan.installmentType === 'auto' ? (
                  <>{currency} {addUserPaymentPlan.finalSellingPrice}</>
                ) : (
                  <Input
