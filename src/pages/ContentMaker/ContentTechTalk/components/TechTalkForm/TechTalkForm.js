...
import { removeFromMessageImage } from '../../../../../actions/contentMaker'
...
class TechTalkForm extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if ((prevState.messageType !== messageType) && formType === 'add') {
      if (!this.state.stickerCode && this.state.messageType === 'sticker') {
        this.setState({ shouldModalVisible: true })
      }
    }
      ...
      if (this.state.messageType === 'sticker') {
        if (this.state.stickerCode) {
          ...
          this.props.addMessages({
            ...args,
            stickerConnectId,
            resetForm: this.resetFormValues
          })
        }
        ...
        this.props.addMessages({ ...args, resetForm: this.resetFormValues })
      }
    } else {
      if (this.state.imageFile && message.image && message.image.uri) {
        await removeFromMessageImage(message.id, message.image.id)
      }
      if (this.state.messageType === 'sticker') {
        if (this.state.stickerCode) {
          const stickerConnectId = get(this.props.stickerEmojis.find(
  ...
  render() {
    const { props } = this
    const { messageType, alignmentType, formFields } = this.state
    return (
        ...
        <Main.FormFooter>
          {props.formType === 'edit' &&
            <Popconfirm
              ...
              placement='topRight'
              onConfirm={() => {
                props.deleteMessages(props.message.id)
              }}
              okText='Yes'
              cancelText='Cancel'
