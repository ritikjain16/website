    ...
    return ['rating5', 'rating4', 'detractors'].map(item => {
      if (item !== 'detractors' && count[item] && count[item].count) {
        return (
          <span
            style={{
              width: `${parseFloat((count[item].count / total) * 100).toFixed(2)}%`,
              paddingBottom: '4px'
            }}
          >
            {count[item].count}
        ...
        item === 'detractors' &&
        ((count.rating1 && count.rating1.count) ||
          (count.rating2 && count.rating2.count) ||
          (count.rating3 && count.rating3.count3))
      ) {
        const detractors = count.rating1.count + count.rating2.count + count.rating3.count
        return (
          <span
            style={{
              width: `${parseFloat((detractors / total) * 100).toFixed(2)}%`,
              paddingBottom: '4px'
            }}
          >
            {detractors}
