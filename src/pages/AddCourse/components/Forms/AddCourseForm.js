    ...
    if ((selectedComponent && selectedComponent.length > 0)
      ...
      if (isSameOrder >= 1) {
        ...
        handleAddCourse(value, meta, thumbnailFile, bannerFile, selectedComponent,
          selectedTargetGroup, theme, selectedLoComponent, codingLanguages, grades)
      }
    } else {
      handleAddCourse(value, meta, thumbnailFile, bannerFile, selectedComponent,
        selectedTargetGroup, theme, selectedLoComponent, codingLanguages, grades)
    ...
    >
      {({ values, handleChange, setFieldValue }) => (
        <Form style={{ padding: '0 10px' }} id='form'>
          ...
          <h3 style={{ marginTop: '10px', marginBottom: '0px' }}>Select Languages :</h3>
          <Select
            mode='multiple'
            labelInValue
            ...
            style={{ width: '100%', margin: '10px 0' }}
            onDeselect={({ label }) =>
                setCodingLanguages(codingLanguages.filter(compo => get(compo, 'label') !== label))}
          >
            {
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: '10px' }}>
            <Button
              ...
              id='add-btn'
              htmlType='submit'
              loading={courseAddStatus && get(courseAddStatus.toJS(), 'loading')}
            >
              Save
            </Button>
          </div>
