...
const SplitScreen = props => {
  ...
  return (
    <Main>
      {(!props.hideScreenLeft || isMobile) && <Main.ScreenLeft className='screenLeft'>{childrenArray[0]}</Main.ScreenLeft>}
      {(!isMobile && !props.hideScreenRight) && <Main.ScreenRight className='screenRight'>{childrenArray[1]}</Main.ScreenRight>}
    </Main>
  )
}
